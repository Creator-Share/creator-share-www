...
export default function SponsorshipEmbedFamilyPage() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (familyList.length > 0) {
            const remaining = (familyList[0].budget_goal - familyList[0].budget_raised) / 100;
            setAmount(remaining);
            setValue([remaining]);
          ...
          setFamilies([]);
          toaster.create({ title: "Error", description: "Failed to load families." });
        }
      } catch {
        setFamilies([]);
        toaster.create({ title: "Error", description: "Failed to load families." });
  ...
  if (!family) {
    return (
      <Flex minH="100vh" align="center" justify="center" direction="column" gap={4}>
        <Text>No families available for sponsorship</Text>
        <Text color="gray.500" fontSize="sm">
          {families.length === 0 ? "No families data found" : "Error loading family data"}
        </Text>
      </Flex>
    );
  }
  ...
  const renderDisclaimer = () => {
    ...
    if ((family.budget_goal - family.budget_raised - amount * 100) > 0) {
      return (
        <>
          This family has a monthly budget goal that must be met for their support.
          {selectedOption === "payment" && (
            <>
              <br />
              Your yearly contribution of ${amount} provides ${monthlyAmount} monthly for this family.
            </>
          )}
          <br />
      ...
      return (
        <>
          This family is partially sponsored. Your contribution will help reach their monthly budget goal!
          {selectedOption === "payment" && (
            <>
              <br />
              Your yearly contribution of ${amount} provides ${monthlyAmount} monthly for this family.
    ...
    return (
      <>
        Your sponsorship will be applied towards the family's monthly budget goals.
        {selectedOption === "payment" && (
          <>
            <br />
            Your yearly contribution of ${amount} provides ${monthlyAmount} monthly for this family.
      ...
      >
          ...
          <Box flex="1">
            <Box w="100%" h="8px" bg="#E8E8EA" borderRadius="md" overflow="hidden" mb={1}>
              <Box
                ...
                bg="#1C3C8C"
                borderRadius="md"
                width={`${Math.min((family.budget_raised / family.budget_goal) * 100, 100)}%`}
                transition="width 0.3s"
              />
            </Box>
            <Flex justify="space-between" mt={1}>
              <Text fontSize="xs" color="gray.500">Raised: ${centsToDollars(family.budget_raised)}</Text>
        ...
        <Flex gap={2} mb={2}>
          <a
            ...
            href={family.username ? `https://dev.creatorshare.com/sponsorships/${family.username}` : undefined}
            target="_blank"
            rel="noopener noreferrer"
            tabIndex={family.username ? 0 : -1}
          >
            More info
          </a>
          <Button
