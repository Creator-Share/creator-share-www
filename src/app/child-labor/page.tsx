...
const ChildLaborMap = dynamic(() => import("./components/ChildLaborMap"), {
  ssr: false,
  loading: () => (
...
const ChildLaborListings = dynamic(() => import("./components/ChildLaborListings"));
const ChildLaborListingsSkeleton = dynamic(() =>
  import("./components/ChildLaborListings/Skeleton").then(
    (mod) => mod.ChildLaborListingsSkeleton
  )
);
...
const SponsorChildLabor = () => {
  ...
  const fetchChildren = React.useCallback(async (filters: Filters) => {
    ...
    try {
      let endpoint = "/api/child-labor/get";
      ...
      if (
        ...
        endpoint = "/api/child-labor/getByAgeAndGender";
    ...
    >
      <Box className="text-center justify-center my-12">
        <Text className="text-[#1C3C8C] font-semibold text-5xl mb-4">
          Help End Child Labor with Creator Share
        </Text>
        <Text className="text-base font-normal text-[#03150E99]">
          Your sponsorship of $39 a month helps rescue children from exploitative labor,
        </Text>
        <Text className="md:px-[200px] text-base font-normal text-[#03150E99]">
          providing them with education, safety, and the chance to reclaim their childhood.
        </Text>
      </Box>
...
export default React.memo(SponsorChildLabor);
