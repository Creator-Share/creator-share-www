...
const ChildLaborCard: React.FC<ChildLaborCardProps> = ({
  people,
  isSelected,
  id,
  onOpenDialog,
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        const response = await fetch(`/api/admin/child-labor/images/${people.id}`);
        if (response.ok) {
  ...
  const handleViewActivity = (e: React.MouseEvent) => {
    e.preventDefault();
    router.push(`/child-labor/${people.username}`);
  };

  const handleSponsorClick = () => {
    console.log(
      `ChildLaborCard: Sponsor button clicked for ${people.name} (ID: ${people.id})`
    );
    if (onOpenDialog) {
      onOpenDialog();
    }
