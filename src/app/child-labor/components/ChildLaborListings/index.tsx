...
import ChildLaborCard from "../ChildLaborCard";
import SponsorDialog from "../SponsorDialog";
import { SponsorPeople } from "@/types";

interface ChildLaborListingsProps {
...
const ChildLaborListings = React.forwardRef<HTMLDivElement, ChildLaborListingsProps>(
  ({ peopleData, selectedChildId, selectedCountry, setSelectedChildId }, ref) => {
    ...
    const handleOpenDialog = (childId: string) => {
      console.log(`ChildLaborListings: Opening dialog for child ID: ${childId}`);
    ...
    const handleDialogNavigation = (direction: "next" | "previous") => {
      ...
      console.log(
        `ChildLaborListings: handleDialogNavigation called with direction: ${direction}, current index: ${currentIndex}`
      );

      if (direction === "next" && currentIndex < visibleChildren.length - 1) {
        const nextChild = visibleChildren[currentIndex + 1];
        console.log(
          `ChildLaborListings: Navigating to next child: ${nextChild.name} (ID: ${nextChild.id})`
        );
        ...
        console.log(
          `ChildLaborListings: Navigating to previous child: ${prevChild.name} (ID: ${prevChild.id})`
        );
...
ChildLaborListings.displayName = "ChildLaborListings";

export default ChildLaborListings;
