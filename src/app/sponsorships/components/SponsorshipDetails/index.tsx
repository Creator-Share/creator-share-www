...
const formatDate = (date: string) => {
  return format(new Date(date), "d MMMM, yyyy h:mmaaa")
    .replace("AM", "AM")
    .replace("PM", "PM")
}
        ...
        <div className="w-full overflow-x-auto">
          <table className="w-full min-w-[600px] overflow-hidden">
            <thead className="bg-[#E4EAE7] border-y-2 border-[#E4EAE7]">
              <tr>
                <th className="text-left text-xs font-normal text-[#6B7772] py-3 px-4">
                  DATE
                </th>
                <th className="text-left text-xs font-normal text-[#6B7772] py-3 px-4">
                  DESCRIPTION
            ...
            <tbody>
              {subscriptions.map((subscription) => (
                <tr
                  key={subscription.id}
                  className="border-y-[1px] border-[#E4EAE7]"
                >
                  ...
                  {!hideStatus && (
                    <td className="py-3 px-4 text-center">
                      <span
                        className={`inline-block rounded-full px-4 py-1 text-xs font-medium ${getStatusStyle(subscription.status)}`}
                      >
