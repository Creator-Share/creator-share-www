...
async function getBeneficiary(username: string): Promise<Beneficiaries | null> {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL || ""}/api/beneficiaries/get/username/${username}`,
      {
        cache: "no-store",
      },
    )
...
export default async function ActivityDetailPage({
  ...
  const activities: Activity[] = await fetchActivitiesByBeneficiaryId(
    beneficiary.id,
  )
  ...
  if (activity?.id) {
    try {
      ...
      if (!mediaErr && Array.isArray(mediaRows) && mediaRows.length > 0) {
        const images = mediaRows.filter(
          (m: MediaRow) =>
            m.type === "IMAGE" || m.type === ("images" as MediaRow["type"]),
        )
        const videos = mediaRows.filter(
          (m: MediaRow) =>
            m.type === "VIDEO" || m.type === ("videos" as MediaRow["type"]),
        )
    ...
    <Box maxW="7xl" mx="auto" mt={8} p={0}>
      ...
      <Box px={{ base: 4, md: 0 }} mb={2}>
        <Text fontSize="sm" color="gray.500">
          <Link href="/sponsorships" className="hover:underline">
            Home
          </Link>{" "}
          /{" "}
          <Link href={`/sponsorships/${username}`} className="hover:underline">
            Child sponsorship
          </Link>{" "}
          /{" "}
      ...
      <Box mb={8} px={{ base: 4, md: 0 }}>
        <Text color="gray.700" mb={4}>
          Raising a teenager can feel like navigating a storm—one moment
          everything is calm, and the next, you’re caught in a whirlwind of
          emotions, attitudes, and change. But beneath that stormy sea is a
          young person trying to figure out who they are and how they fit into
          the world. Your job as a parent isn’t to control the storm, but to be
          the steady lighthouse they can rely on.
        </Text>
        <Text
          as="span"
          fontWeight="bold"
        ...
        <Text color="gray.700" mb={4}>
          Teen brains are still developing—especially the prefrontal cortex,
          which governs decision-making and impulse control. This means they may
          act impulsively, take risks, or seem emotionally volatile.
          Understanding this can help you approach conflict with more empathy
          and patience.
        </Text>
        <Text
          as="span"
        ...
        <Text color="gray.700" mb={4}>
          Kids’ wishes to push for independence. They still need boundaries. Set
          clear, consistent rules and consequences—but involve them in the
          process. When teens feel heard, they’re more likely to cooperate and
      ...
      <Box mb={8} px={{ base: 4, md: 0 }}>
        ...
        <Text color="gray.700" mb={4}>
          The days of chatting freely might never pass, but communication hasn’t
          stopped—it’s just changed. Instead of grilling them, ask open-ended
          questions. Listen more than you talk. Be curious, not judgmental.
