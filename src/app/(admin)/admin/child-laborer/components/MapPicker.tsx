...
const MapPicker: React.FC<MapPickerProps> = ({
  ...
  const fetchLocationData = async (lat: number, lng: number) => {
    try {
      const response = await fetch(`/api/proxy/nominatim?lat=${lat}&lon=${lng}`)
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (location) {
        ...
        const { locationStr, country } = await fetchLocationData(
          location[0],
          location[1],
        )
        ...
        toaster.create({
          ...
          description:
            "Could not find the specified location. Try a different search term or format, or click directly on the map.",
          duration: 5000,
        })
      }
    } catch (error) {
      ...
      <Field label="Location">
        ...
        {isError && (
          <Text color="red.500" fontSize="sm" mb={2}>
            Location not found. Try a different search term or click directly on
            the map.
          </Text>
        )}
        ...
        {selectedLocation && (
          <Text mt={2} fontSize="sm" color="gray.600">
            Selected Location: {selectedLocation[0].toFixed(5)},{" "}
