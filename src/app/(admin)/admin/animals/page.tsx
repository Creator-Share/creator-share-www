...
const AnimalsTable = () => {
    ...
    async function fetchData() {
      try {
        const response = await fetch(
          "/api/admin/beneficiaries/retrieve?beneficiary_type=ANIMAL",
        )
        if (!response.ok) {
          throw new Error("Network response was not ok")
        }
        const fetchedData = await response.json()
        const animalsArr = Array.isArray(fetchedData.beneficiaries)
          ? fetchedData.beneficiaries
          : []
        setData(
          animalsArr.map((animal: AnimalBeneficiary) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!response.ok || !responseData.beneficiary) {
        throw new Error(
          responseData.error || "Failed to create animal beneficiary",
        )
      }
      ...
      setData((prev: AnimalBeneficiary[]) => [
        ...prev,
        {
          ...
          budget_goal: String(
            dollarsToCents(
              typeof beneficiary.budget_goal === "string"
                ? parseInt(beneficiary.budget_goal)
      ...
      if (videoFiles.length > 0 && beneficiary.id) {
        try {
          ...
          if (data?.public_url) {
            ...
            setData((prev: AnimalBeneficiary[]) =>
              prev.map((a) =>
                a.id === beneficiary.id ? { ...a, video_url: publicUrl } : a,
              ),
            )
            setImageFiles([])
            setVideoFiles([])
  ...
  const handleSave = async (updatedAnimal: AnimalBeneficiary) => {
    try {
      ...
      const response = await fetch(
        `/api/admin/beneficiaries/update/${updatedAnimal.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDelete = async (animalId: string) => {
    try {
      const response = await fetch(
        `/api/admin/beneficiaries/delete/${animalId}`,
        {
          method: "DELETE",
        },
      )
  ...
  return (
    <Box className="container mx-auto h-[calc(100vh-200px)] mt-12">
      ...
      <DeleteDialog
        ...
        onConfirm={async () => {
          const animalIds = selectedRowsForDeletion.map(
            (row) => row.original.id,
          )
          try {
            const response = await fetch(
              "/api/admin/beneficiaries/bulk-delete",
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
            ...
            setData((prev: AnimalBeneficiary[]) =>
              prev.filter((a) => !animalIds.includes(a.id)),
            )
