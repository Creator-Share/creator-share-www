...
interface EditDrawerProps {
  selectedChild: ChildLabor;
  formData: ChildLabor;
  setFormData: React.Dispatch<React.SetStateAction<ChildLabor>>;
  isDrawerOpen: boolean;
  onClose: () => void;
  onSave: (updatedChild: ChildLabor) => Promise<void>;
...
const EditDrawer: React.FC<EditDrawerProps> = ({
  ...
  }, [selectedChild, setFormData]);

  const handleSave = async () => {
    const requiredFields = ["name", "age", "location", "workType", "status"] as const;
    const emptyFields = requiredFields.filter(
      (field) => !formData[field as keyof ChildLabor]
    );
    ...
    try {
      ...
      toaster.create({
        title: "Success",
        description: "Child labor case updated successfully",
        duration: 5000,
      });
    } catch (error) {
      ...
      toaster.create({
        title: "Error",
        description: "Failed to update child labor case",
        duration: 5000,
      });
    } finally {
      setIsEditing(false);
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete this child labor case?")) {
      try {
        await onDelete(selectedChild.id);
        toaster.create({
          title: "Success",
          description: "Child labor case deleted successfully",
          duration: 5000,
        });
      } catch (error) {
        ...
        toaster.create({
          title: "Error",
          description: "Failed to delete child labor case",
          duration: 5000,
        });
      }
    }
  ...
  return (
    <DrawerRoot open={isDrawerOpen} onOpenChange={({ open }) => !open && onClose()}>
      ...
      <DrawerContent>
        <DrawerHeader>
          <DrawerTitle>
            <Text fontSize="5xl">Edit Child Labor Case</Text>
          </DrawerTitle>
        </DrawerHeader>
        <DrawerBody>
          <Fieldset.Root size="lg">
            ...
            <Fieldset.Content>
              ...
              <Field label="Work Type" required errorText="This field is required">
                <Input
                  name="workType"
                  className="border"
                  px={2}
              ...
              <Field label="Status" required errorText="This field is required">
                <NativeSelectRoot>
                  <NativeSelectField
                    ...
                    onChange={(e) => setFormData(prev => ({ ...prev, status: e.target.value as ChildLabor['status'] }))}
                  >
