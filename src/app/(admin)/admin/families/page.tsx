...
type TableInstance = {
  getSelectedRowModel: () => { rows: Row<Family>[] };
...
const FamiliesTable = () => {
  ...
  const [selectedRowsForDeletion, setSelectedRowsForDeletion] = useState<Row<Family>[]>([]);
  ...
  const handleSave = async (updatedFamily: Family) => {
    try {
      // TODO: Replace with actual API endpoint
      setData((prevData) => prevData.map(family =>
  ...
  const confirmDelete = async () => {
    const familyIds = selectedRowsForDeletion.map((row: Row<Family>) => row.original.id);

    try {
      // TODO: Replace with actual API endpoint
      setData((prevData) =>
        prevData.filter((family) => !familyIds.includes(family.id))
      );
      if (tableRef.current) {
      ...
      toaster.create({
        title: "Success",
        description: "Selected families deleted successfully.",
        duration: 5000,
      });
    } catch (error) {
  ...
  const handleDelete = async (familyId: string) => {
    try {
      // TODO: Replace with actual API endpoint
      setData((prevData) => prevData.filter(family => family.id !== familyId));
      setIsEditDrawerOpen(false);
    } catch (error) {
  ...
  return (
    <Box className="container mx-auto h-[calc(100vh-200px)] mt-12">
      <Box className="grid grid-cols-2 mb-2">
        <Text className="text-3xl font-semibold leading-9">Families</Text>
      ...
      <DataTable
        ...
        onRowClick={(data: unknown) => {
          setSelectedFamily(data as Family);
          setIsEditDrawerOpen(true);
        }}
      />
      {isEditDrawerOpen && selectedFamily && (
        <EditDrawer
