...
const StreetChildMap = dynamic(() => import("./components/StreetChildMap"), {
  ssr: false,
  loading: () => (
...
const StreetChildListings = dynamic(() => import("./components/StreetChildListings"));
const StreetChildListingsSkeleton = dynamic(() =>
  import("./components/StreetChildListings/Skeleton").then(
    (mod) => mod.StreetChildListingsSkeleton
  )
);
...
const SponsorStreetChild = () => {
  ...
  const fetchChildren = React.useCallback(async (filters: Filters) => {
    ...
    try {
      let endpoint = "/api/street-involved/get";
      ...
      if (
        ...
        endpoint = "/api/street-involved/getByAgeAndGender";
    ...
    >
      <Box className="text-center justify-center my-12">
        <Text className="text-[#1C3C8C] font-semibold text-5xl mb-4">
          Help Street-Involved Children with Creator Share
        </Text>
        <Text className="text-base font-normal text-[#03150E99]">
          Your sponsorship of $39 a month helps provide shelter, safety, and support
        </Text>
        <Text className="md:px-[200px] text-base font-normal text-[#03150E99]">
          to children living on the streets, giving them a chance for a better life and a brighter future.
        </Text>
      </Box>
...
export default React.memo(SponsorStreetChild);
