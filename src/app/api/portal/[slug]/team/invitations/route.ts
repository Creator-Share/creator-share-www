...
function classifyFailure(error: unknown): { status: number; code: string } {
  ...
  switch (postgresCode) {
    ...
    case "23505":
      return { status: 409, code: "invitation_conflict" }
    case "55000":
    case "23514":
      return { status: 409, code: "lifecycle_conflict" }
    default:
      return { status: 500, code: "invitation_failed" }
  }
}
...
export async function POST(
  ...
  try {
    ...
    return response(
      { ok: false, code: "invitation_failed", requestId: context.requestId },
      500,
    )
  }
  ...
  const input =
    rawBody === null ? null : parseAdvocateInvitationIssueInput(rawBody)
  if (input === null) {
    return response(
      { ok: false, code: "invalid_request", requestId: context.requestId },
      400,
  ...
  try {
    const repository = createAdvocateInvitationRepository({
      authenticatedClient,
    ...
    const pending = await repository.loadPending(portal.advocateId)
    const invitation =
      pending.find(
        (candidate) => candidate.invitationId === issued.invitationId,
      ) ?? null
    if (invitation === null || invitation.expiresAt !== issued.expiresAt) {
