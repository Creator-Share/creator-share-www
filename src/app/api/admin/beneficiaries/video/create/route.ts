...
export async function POST(req: Request) {
  ...
  try {
    ...
    const beneficiaryId = (formData.get("beneficiaryId") ??
      formData.get("beneficiary_id")) as string
    ...
    if (insertErr) {
      console.error(
        "Beneficiary video upload error: DB insert failed:",
        insertErr,
      )
    ...
    if (uploadErr) {
      console.error(
        "Beneficiary video upload error: storage upload failed:",
        uploadErr,
      )
    ...
    try {
      ...
      const existingMetadata =
        (existingBeneficiary && existingBeneficiary.metadata) || {}
      const updatedMetadata = {
        ...existingMetadata,
        video_media_id: mediaRow.id,
      }
      ...
      if (updateError) {
        console.error(
          "Beneficiary video upload error: failed to persist media reference:",
          updateError,
        )
      ...
      console.error(
        "Unexpected error updating beneficiary metadata with media id:",
        e,
      )
    ...
    return NextResponse.json(
      { media: mediaRow, public_url: publicUrl },
      { status: 200 },
    )
  } catch (err: unknown) {
