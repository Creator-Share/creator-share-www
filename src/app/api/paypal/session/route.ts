  ...
  try {
    ...
    const formattedResponse = {
      session: {
        metadata: {
          type: 'sponsorship',
          childName: orderData.purchase_units[0]?.description?.split(' for ')[1] || 'your sponsored child',
          childLocation: orderData.purchase_units[0]?.shipping?.address?.country_code || '',
          amount: (parseFloat(orderData.purchase_units[0]?.amount?.value || '0') * 100).toString(),
          paymentType: orderData.purchase_units[0]?.description?.includes('Monthly') ? 'subscription' : 'yearly'
        },
        customer_details: {
