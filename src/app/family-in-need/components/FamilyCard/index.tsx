...
const FamilyCard: React.FC<FamilyCardProps> = ({
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        const response = await fetch(`/api/admin/families/images/${people.id}`);
        if (response.ok) {
  ...
  const handleViewActivity = (e: React.MouseEvent) => {
    e.preventDefault();
    router.push(`/family-in-need/${people.username}`);
  };

  const handleSponsorClick = () => {
    console.log(
      `FamilyCard: Sponsor button clicked for ${people.name} (ID: ${people.id})`
    );
    if (onOpenDialog) {
      onOpenDialog();
    }
  ...
  return (
    <Box
      ...
      } md:mb-0 ${isSelected ? "highlight-family" : ""}`}
            ...
            >
              <Flex align="center" gap={2} mb={4}>
                ...
                <Text fontSize="sm" className="text-gray-500">
                  Family of {people.family_size || "Unknown"} members
                </Text>
              </Flex>
