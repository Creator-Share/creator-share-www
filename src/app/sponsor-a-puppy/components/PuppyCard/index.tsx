...
const PuppyCard: React.FC<PuppyCardProps> = ({
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        const response = await fetch(`/api/admin/puppies/images/${people.id}`);
        if (response.ok) {
  ...
  const handleViewActivity = (e: React.MouseEvent) => {
    e.preventDefault();
    router.push(`/sponsor-a-puppy/${people.username}`);
  };

  const handleSponsorClick = () => {
    console.log(
      `PuppyCard: Sponsor button clicked for ${people.name} (ID: ${people.id})`
    );
    if (onOpenDialog) {
      onOpenDialog();
    }
  ...
  return (
    <Box
      ...
      } md:mb-0 ${isSelected ? "highlight-puppy" : ""}`}
