...
Two places where a deliberate, compiling change to production TypeScript passes the entire required suite.
...
| Session completion never checked that an existing attribution identity cookie belonged to the account that just authenticated, so a shared browser could keep user A's signed identity while user B was authenticated | `tests/advocates/attribution-identity-completion.spec.ts`        |
...
| --- | ----------- | ------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | correctness | `src/app/api/auth/attribution-identity/route.ts`              | The account-ownership term is now asserted, but the two sibling terms in the same condition are not. `requiresNormalization` and `requiresRefresh` can each be removed with the suite still green. Two candidate tests were written and then withdrawn because they passed without catching either mutation, and the reason could not be established: a direct probe of the cookie resolver returned a null signal for both a duplicated and a single freshly minted cookie, which does not match the behaviour the route sees. Close this by first determining what the route's environment makes the resolver return, then constructing a case where the signal is non-null and exactly one of the two flags is set. |
