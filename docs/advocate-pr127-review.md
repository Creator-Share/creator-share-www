...
The structural replay for the atomic deletion candidate adds exactly one function and changes no existing catalog entry. All eight representative legacy-data projections remain equal. The existing media storage policy checks global administrator authority without requiring a surviving media row, so post-commit cleanup remains permitted. A copy correction removes an unsupported instruction to archive a child; the error now states that financial history prevents deletion. All five route contracts still pass.

The first hosted atomic-deletion run (35194644035) passed pgTAP, then failed before the first concurrency barrier: the disposable schema-only database lacked the PayPal provider-account seed row. The harness correctly refused to insert a catalog reference with a missing parent. The shared and inline fixtures now explicitly seed that account without overwriting an existing one. No production constraint was relaxed. Hosted validation must be repeated; the failure is retained as evidence. WebKit run 35194644036 passed.

## Atomic deletion hosted validation complete
...
## Remove unused browser reservation state

Repository caller inspection found no reader of the sponsorship-in-progress context. Its sole consumer only cleared a beneficiary flag when the modal closed; no caller ever set a flag true. Removed that context, its global provider wrapper, and the clearing effect, eliminating 80 net application lines and unnecessary local-storage writes. Server-owned intents, checkout operation recovery, and the legacy reservation table remain intact.

Removed the unreferenced 292-line REALTIME_IMPLEMENTATION.md report, which described an active useReservations hook, ReservationsProvider, and reservation endpoints already absent from this branch. That document also claimed an offline fallback the remaining code did not implement. No currently implemented product capability was removed. TypeScript, lint, and all 1,607 selected server-free tests pass. Hosted checkout and browser validation is pending for this application cleanup.

## Delegate account-state extension

The adjacent direct-read boundary had the same defect: a normal authenticated delegate retained portal.view permission, a tenant base-table row, and one get_my_advocate_portal_access result after an Auth ban. The reproduction used the complete schema and normal RLS reads. Reusing the account-state helper in the delegate view predicate and portal-list RPC changes those results to false, zero, and zero while retaining active access.
...
Run 35202180401 passed the corrected dispute-recovery test and the new sponsor account-state assertions. Two preexisting role-compatibility assertions failed because they invoked sponsor reads before inserting their Auth account fixture. Those assertions now run after that insertion, with unchanged expectations and unchanged assertion count. Other compatibility checks retain their original fixture timing. The direct profile self-read policy also receives the existing active-account guard, with before/after normal-role proof and two additional database assertions plus HTTP coverage. No public role catalog or public beneficiary read is restricted by this change.
