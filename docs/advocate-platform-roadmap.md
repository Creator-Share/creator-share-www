...
The MVP does not yet add a capability-free redemption receipt for a response lost after successful initial-owner activation. FF-042 tracks a stable client operation ID, immutable sanitized outcome, exact replay, mismatched-input rejection, and recovery with no capability or owner contact retained. Until that receipt exists, the client must not automatically retry an ambiguous redemption after capability consumption.
