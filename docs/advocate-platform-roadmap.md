...
Password recovery uses the same proof standard through a separate canonical apex flow. The request accepts one normalized email, reserves public delivery capacity before the shared proof issuer, and returns the same accepted response for every account, quota, gate, and provider disposition. The browser keeps the email and code only in React memory. It never places contact or proof material in a URL, referrer, cookie, local storage, session storage, history state, analytics, or log. A separate entry form supports a fresh tab or device without weakening that storage boundary.

Successful recovery verification must establish a purpose-specific host-only Supabase session cookie and record the exact verified user and Auth session in the existing 15 minute server receipt. Production uses a Secure `__Host-` cookie name with root path, no Domain attribute, HttpOnly, and SameSite Lax. This prevents any advocate sibling from planting or shadowing the password recovery session. The password mutation route requires that exact cookie, atomically consumes the exact receipt before one password update, signs out the recovery session, proves the response clears the cookie, and clears the attribution identity signal only after full success. Missing, expired, duplicate, wrong-user, wrong-session, ordinary-session, and replayed authority fail closed. An uncertain browser response never authorizes a mutation by itself.
...
- `dev`, `development`, `staging`, `stage`, `test`, `preview`, `local`, and `advocate-staging`.
...
The invitation proof migration and matching application form one coordinated, forward-only release unit. Production provider execution for this feature is not authorized yet, so the expected legacy attempted-outbox count is zero. A nonzero count is an investigation stop. Before promotion, retain protected evidence from the exact hosted Supabase project that `mailer_otp_exp` is no more than 3,600 seconds. Supabase documents that Email OTP Expiration also controls Magic Links and invitation links in [Passwordless email logins](https://supabase.com/docs/guides/auth/auth-email-passwordless) and [Users and invitation links](https://supabase.com/docs/guides/auth/users). Local `supabase/config.toml` cannot prove hosted state. Keep the worker disabled if the setting is unknown or greater than 3,600 seconds. Verify separately that the production Vercel plan supports the declared 120 second function duration. The worker reserves a ten second platform buffer through an internal 110 second budget. See [Vercel function maximum duration](https://vercel.com/docs/functions/configuring-functions/duration).

Freeze approved manual callers and external schedulers, deploy a temporary dedicated `ADVOCATE_INVITATION_EMAIL_WORKER_SECRET` that Vercel Cron does not send, and prove a request authenticated with `CRON_SECRET` receives 401 before any invitation claim. That response does not drain a legacy invocation already running. Wait at least 70 seconds, which exceeds the legacy worker's 60 second maximum by ten seconds, and confirm through Vercel invocation telemetry that no prior worker remains active. Retain only aggregate counts for `processing` rows with and without `delivery_started_at`. Do not proceed while a pre-handoff row has an unexpired five-minute claim lease. Treat a begun handoff as potentially delivered. Classify it from provider evidence, revoke it when delivery is rejected or remains inconclusive, or postpone cutover. The intentionally invalid `worker_configuration` preflight is pause evidence only.

Apply the migration while delivery remains disabled. It removes the legacy four-argument claim function and installs a version 1 shared-issuer claim that remains closed. After migration commit, call the service-only `public.arm_advocate_invitation_legacy_email_proof_quarantine(context_request_id uuid, context_trace_id text DEFAULT NULL) RETURNS timestamptz`. Commit its immutable server-owned `legacy_claim_fenced_at`, wait the database-enforced 70 seconds, then call `public.quarantine_legacy_advocate_invitation_proofs(verified_provider_otp_expiry_seconds smallint, context_request_id uuid, context_trace_id text DEFAULT NULL)`. The quarantine returns exactly `candidate_outbox_count`, `unique_recipient_count`, `quarantined_outbox_count`, `created_gate_count`, `preserved_gate_count`, `fence_expires_at`, and `executed_at`. Same-expiry replay returns the original contact-free fields without extending the fence, while changed-expiry replay fails closed. Only the committed receipt opens version 1 claims. The verified hosted expiry from 1 through 3,600 seconds is retained as release evidence and binds replay, but it cannot shorten safety. The quarantine covers every possibly live legacy proof across all invitation and outbox states and checks pre-outbox `last_sent_at`. It fails atomically on recent redaction, a recent prior send without a retained recipient HMAC, incompatible normalization or HMAC versions, or any unexpected count. Otherwise it always applies the fixed one-hour maximum proof lifetime plus the fixed 300 second safety margin, exactly 3,900 seconds, to both the legacy-evidence blocker and every recipient fence. It permanently marks unresolved attempted outboxes and records the immutable receipt. Recovery for a permanently quarantined row is explicit revocation and reissue after its recipient fence expires.

Quarantine is a one-time maintenance pause for all invitation reads and writes. Invoke the arm and quarantine RPCs through separate protected transactions, and invoke quarantine as a standalone autocommit RPC or as the only operation in a dedicated transaction. Set a 5 second `lock_timeout` and a 60 second `statement_timeout` for the session or dedicated transaction before invoking quarantine so receipt, recipient-gate, and audit work remains bounded. The function also self-bounds its individual lock waits at 5 seconds, but only the caller can establish the 60 second limit before the outer statement begins. A first quarantine execution acquires `ACCESS EXCLUSIVE` locks on `public.advocate_invitations` and `public.advocate_invitation_email_outbox` in one `NOWAIT` statement before scanning invitation evidence or changing cutover state. Exact same-expiry replay returns the immutable receipt without taking those table locks because it performs no cutover change. If either table is already in use during first execution, PostgreSQL returns SQLSTATE `55P03` and no cutover mutation occurs. On any error or timeout in a dedicated transaction, issue an immediate `ROLLBACK`; an autocommit call rolls back automatically. Invitation reads and writes that begin after a successful quarantine lock acquisition may queue briefly until that transaction commits. A `55P03`, lock timeout, or statement timeout is a drain-invariant warning and an investigation stop. Stop and investigate every failure. Reconfirm through telemetry and database state that no retired worker, reader, or invitation mutation remains before retrying. Do not retry blindly or include unrelated work in the quarantine transaction.

The combined database release gate adds `supabase/tests/advocate_invitation_email_proof_settlement.test.sql` plus `yarn test:advocate:invitation-proof-quarantine-concurrency` on the same fresh loopback-only Supabase stack. It uploads a separate sanitized artifact proving the committed arm-before-wait sequence, the database-enforced 70 second drain, immutable exact replay, changed-expiry rejection, version 1 claim closure until quarantine commits, exact recipient-fence boundaries, and quarantine-first serialization against invitation creation, claim, settlement, revocation, and retention work. It also proves that an ordinary reader or a prior settlement, revocation, or lifecycle writer causes exact fail-fast SQLSTATE `55P03`, leaves no partial cutover mutation, and permits a clean quarantine retry after the competing transaction ends. The evidence contract permits only scenario names, fixed outcome categories, aggregate counts, migration provenance, and pass state. Contact, recipient HMACs, credentials, row identifiers, operation UUIDs, trace values, and raw audit material are forbidden.

The compatible worker uses the outbox UUID as the shared-gate operation ID and worker request context as issuer context. It never stores proof material. Coalesced, deferred, configuration unavailable, acquire unavailable, begin unavailable, and pre-issuer deadline outcomes call neither provider nor SMTP and refund the delivery attempt. Begin uncertainty still waits the exact 3,900 second proof fence. Provider ambiguity and issued-not-handed-off outcomes retain the attempt and wait the exact fence. No shared-issuer delay receives an exponential multiplier. Issued target mismatch settles an immutable terminal receipt. Batch size must equal concurrency, the five-minute database lease remains authoritative, and the worker checks headroom before issuance, after issuance, and immediately before delivery begin.

Deploy only after the quarantine receipt commits. Validate one new account and one existing account through the dedicated worker secret, remove the override in a new deployment, verify normal Cron delivery resumes, and collect the configured release preflight. On rollback, retain the migration and quarantine receipt. Never restore the removed claim signature or direct proof worker. Keep delivery disabled until a compatible shared-issuer build returns. Already issued invitation acceptance remains independent of the worker pause.
...
The advocate Domain administration section is a read-only tenant surface over the existing authenticated access projection. It requires `portal.domains.view` and exposes only the assigned Creator Share hostname, normalized domain and publication states, safe lifecycle guidance, and the public link when relationship, publication, and domain are all active. Provider identifiers, provider errors, job state, canary evidence, credentials, and reconciliation internals remain Creator Share private. Domain provisioning and repair continue to run through the automated platform workflow, so advocate delegates receive status without acquiring provider authority.

Branding and logo mutations revalidate authority inside the service-role transaction that performs the write. After taking the advocate root lock, the boundary locks a healthy confirmed nonanonymous auth account, the active tenant membership, and the exact branding role and permission evidence. Those locks remain held through logo reservation or branding commit, so account bans, membership suspension, and role revocation serialize to one defensible ordering and a losing mutation leaves no reservation, version change, or audit residue. The database wrappers bound lock acquisition to five seconds. The ordinary branding route gives the service client a fifteen second request budget. SQLSTATE `55P03` becomes a safe retryable conflict, never an unbounded request. FF-048 supplies disposable database proof for both transaction orderings, timeout residue and retry, migration provenance, sanitized evidence, and normal-run transient database cleanup. A separate child process gate injects `SIGINT` and `SIGTERM` after the real harness holds an open transaction and a server-observed blocked waiter, requires exit codes 130 and 143, and proves that neither a database nor false pass evidence survives. Each child owns a unique validated database prefix, so emergency cleanup cannot affect another harness. The complete local acceptance matrix and the hosted authority gate on commit `9b387f1e` pass, so FF-048 is complete. Browser authentication is an initial check, not the database mutation authority.
...
- Provider-free PostgREST role mechanics gate through the real loopback Kong, Auth, and PostgREST stack. Pair the 28-assertion pgTAP suite with the 90-case HTTP matrix across legacy and current service transports, canonical and browser publishable transports, a real authenticated sponsor session, anonymous callers, ordinary-user denials, missing credentials, exact cleanup, sanitized version-bound evidence, and forced termination cleanup.
- Hosted exact-project PostgREST role canaries for both service key formats, authenticated sponsor calls, and anonymous and ordinary-user deny controls. The local gate remains supporting mechanics evidence and cannot satisfy this deployed release boundary.
...
The provider-free attribution settlement integration gate now exercises one tenant through the current durable publication operation, lease-fenced canonical report completion, short-lived deployment capability, signed-session approval, five direct sponsorships, and five post-visit sponsorships. Its publication report contains test-supplied provider-free evidence that conforms to the canonical evidence schema. It then uses the version-two server-owned checkout operation and receipt boundary, production post-verification gateway-event ingestion with test-supplied verified facts, authoritative settlement, final attribution, private analytics, weekly public metric release, public presentation, and welcome claim materialization. Test-only time travel backdates already-settled immutable facts so daily and weekly cutoff functions can observe them. No production database behavior is replaced in the post-verification sponsorship settlement chain. This gate does not verify provider network calls, Stripe webhook signatures, or physical browser behavior. Hosted gateway canaries, detailed publication evidence suites, and focused application tests remain separately mandatory.

The provider-free PostgREST compatibility gate now binds the claim model to the same loopback network boundary used by the application instead of relying only on manually injected database settings. Its 28 pgTAP assertions cover all five shared service guards, including public metric release. Its 90 HTTP cases use the installed Supabase JavaScript client for the production server and browser header shapes, canonical raw transports for current keys, one real Auth-issued sponsor token with both public key generations, anonymous and missing-credential controls, and exact pre-mutation errors for all ten protected RPCs. The provenance check hashes the raw migration files and compares the running migration history with a disposable ledger parsed by the pinned Supabase CLI, so edited same-version SQL cannot be attributed to a stale local schema. Evidence is emitted only after bounded exact-email cleanup, records no credential, user, URL, request body, or raw message, and remains explicitly local mechanics only. A separate termination gate proves isolated Auth fixture cleanup under repeated signals and proves that evidence-writing interruption removes stale final and partial artifacts for both `SIGINT` and `SIGTERM`. The deployed exact-project matrix is still mandatory before promotion.
...
Identical checkout behavior on a branded advocate subdomain is now proven in a real browser rather than inferred from module tests. `yarn test:advocate:checkout-browser-parity` builds an isolated production fixture that renders only the production `SponsorshipModal`, then drives four full checkouts: Stripe and PayPal, each on a primary origin and an advocate subdomain origin. It asserts same-origin initiation, a request body identical across origins apart from a distinct version 4 checkout operation identifier per checkout, a genuine top-level document navigation to the provider, and session storage holding only the noncontact operation binding and opaque receipt with no sponsor contact or provider URL. The gate is documented in `docs/advocate-payment-release-runbook.md` and runs in the advocate publication database workflow.
...
The MVP uses an apex-origin attribution broker. Advocate visits send one bounded credentialed request to the fixed `https://creatorshare.com/api/advocates/exposure` endpoint. The broker derives the tenant only from an exact one-label `Origin`, verifies that exact hostname through the service-only active presentation snapshot, and grants credentialed CORS only to that origin. A qualifying POST then records the visit against an authenticated, host-only apex visitor token. A later primary-site checkout can use that same apex token without exposing it to every sibling host. Stripe and PayPal checkout APIs remain same-origin on each tenant host. Direct attribution is derived on the server from the exact checkout Host and does not require the exposure visitor digest to match.

The isolated staging deployment is the only alternate broker topology. It activates only when the canonical base URL is exactly `https://advocate-staging.creatorshare.com`. Its primary broker is that exact origin, and its sole initial tenant host is `canary.advocate-staging.creatorshare.com`. The request host maps to the canonical database hostname `canary.creatorshare.com`, preserving the production schema contract. Staging is hard-bound to Supabase project `destjwstohzmufshfnuy` at exact origin `https://destjwstohzmufshfnuy.supabase.co`. Build-time and shared runtime client checks reject any other project before Supabase access. Without the exact deployment configuration, both staging hosts remain closed. Arbitrary configured roots, other one-label tenants, additional nested labels, and production invitation links cannot opt into this path. Provider automation must remain disabled in this environment. The code rejects active provider mode because provisioning records retain production-shaped canonical hostnames and must never direct a staging worker toward production DNS or domain resources. LLM and Telegram credentials are forbidden in staging until separately scoped integrations exist. Application email is restricted to the Ethereal sandbox with mandatory STARTTLS and canary-only recipients. Every application flow using the shared Supabase Auth proof issuer applies the same recipient fence before quota acquisition, issuer gate work, or any Supabase Auth provider call. Checkout accepts only Stripe test keys and the PayPal sandbox API. `NEXT_PUBLIC_MAPTILER_KEY` is forbidden, and exact staging uses the OpenStreetMap fallback. Vercel Analytics is disabled whenever the exact staging base URL is active. Vercel's generated production hostname admits only the declared authenticated worker paths, so scheduled work can run without reopening public routes or creating durable cookies.

Exact advocate staging requires a dedicated Vercel project before any deployment. It must not reuse the existing Creator Share production project binding, environment values, schedules, generated production hostname, or domain ownership. A copied local binding to the production project is a deployment stop until the worktree is relinked to the dedicated staging project.

Blind sponsorship auto-match remains known pre-existing debt. The Stripe webhook currently self-calls an administrative route that requires a live Creator Share super administrator browser session, so the webhook cannot perform the match autonomously. Exact staging now keeps that self-call on the staging origin, but this origin correction does not repair the authorization contract. Automated matching must move behind an explicit service-authorized boundary with its own audit and replay controls before it can be counted as validated behavior.

Middleware bypasses generic session refresh and visitor issuance for the exact broker pathname on every Host. Tenant, alias, malformed, reserved, and unmanaged copies therefore reach the route without any preliminary durable cookie mutation. The route itself accepts state changes only on the exact canonical apex Host. It requires the fixed protocol version, JSON content type, a strict qualifying page path, same-site CORS metadata on POST, and no client-supplied advocate, visitor, authentication, or referrer identity. Invalid requests receive no CORS authority and no durable mutation. Database and network failures remain retryable without marking the page complete.

The production-shaped HTTPS feasibility gate passed in Chrome and WebKit. The first exact managed-origin request set a `Secure`, `HttpOnly`, `SameSite=Lax`, host-only apex cookie. The second request sent it. Tenant JavaScript could not read it, and an untrusted sibling was rejected. A separate live browser protocol test confirms that both engines send the exact custom preflight headers and complete Fetch Metadata on OPTIONS and POST when the initiating context is potentially trustworthy. Both requests therefore require the full site, mode, and destination contract. CORS, Origin, and Fetch Metadata constrain browsers but do not authenticate a nonbrowser client. Event-key deduplication, payment-backed conversion semantics, Cloudflare controls, and analytics abuse monitoring remain part of the threat model.

The durable broker protocol and adversarial application tests are implemented. Release still requires a production mobile Safari canary, tracking-prevention coverage, exact deployed-domain verification, and end-to-end exposure through primary checkout attribution and settlement evidence. Browsers classify tenant localhost and apex localhost as cross-site even though production Creator Share subdomains are same-site. Local development accepts that exact cross-site value only in development and cannot substitute for the production-shaped cookie and staff-exclusion canaries.
...
- The apex attribution broker retains the passed Chrome and WebKit production-shaped feasibility evidence and the implemented adversarial protocol tests, then passes mobile production, tracking-prevention, abuse-monitoring, and settlement canaries before post-visit attribution is enabled across hosts.
...
- Autonomous blind sponsorship auto-match through a service-authorized internal boundary. The pre-existing webhook self-call still targets a super administrator browser route and is not staging validation evidence.
