...
This database migration and the matching application routes form one coordinated release unit. The new application cannot call the previous redemption signature, while a previous initial-owner caller cannot provide the operation required by the new boundary. Because initial-owner browser onboarding is first introduced by this release, apply the migration before enabling invitation delivery, then promote the matching application build and validate it before sending any link. Legacy delegate callers remain temporarily compatible when they omit the operation, but every browser delegate acceptance uses the receipt path. Any later one-sided rollout requires an explicit compatibility or maintenance gate first.
