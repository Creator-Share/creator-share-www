...
The first Advocate release installs the current shared email proof issuer directly. The owner confirmed on September 17, 2026 that none of the Advocate migrations has been applied to staging or production. The Advocate invitation outbox is created by this migration series, so there is no previous Advocate outbox to drain or quarantine. Do not deploy historical intermediate checkpoints. If target inspection finds any previously applied Advocate migration or an existing Advocate invitation outbox, stop and reconcile that state before applying this consolidated series.

Before enabling delivery, retain protected evidence that the exact hosted Supabase project's `mailer_otp_exp` is no more than 3,600 seconds. Local `supabase/config.toml` is not hosted evidence. Keep delivery disabled if expiry is unknown or exceeds the shared issuer's proof-lifetime bound. Verify that the production Vercel plan supports the 120 second function duration and its internal 110 second work budget. The hosted proof-supersession canary remains a release gate.

Keep `ADVOCATE_INVITATION_EMAIL_WORKER_SECRET` distinct from `CRON_SECRET` until delivery validation is authorized. Verify that the ordinary scheduler credential receives `401` before any claim. Apply the complete reviewed schema and deploy the matching application. Validate one new-account and one existing-account invitation through the dedicated worker credential before enabling normal Cron delivery. Preserve the shared issuer's recipient fences, exact 3,900 second uncertainty delays, immutable settlement receipts, five-minute leases, and SMTP ambiguity quarantine. Batch size must equal concurrency. Rollback disables delivery until a compatible application is available; it does not restore a retired worker or claim signature.
