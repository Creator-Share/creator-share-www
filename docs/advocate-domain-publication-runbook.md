...
7. Send an unscheduled `POST` with no body to the staged deployment's immutable generated URL at `/api/internal/advocates/release-preflight` with the exact `Authorization: Bearer <CRON_SECRET>` header. Retain only its fixed categorical result. Never record the request header or secret. Require every check unrelated to parent-domain cookies, including `deployment_identity` and `provider_automation_gate`, to be `configured`. Require `cross_subdomain_cookie_trust`, `cross_subdomain_cookie_trusted_collector`, and `cross_subdomain_cookie_fresh_provider_evidence` to remain `unverified`, with active mode and policy digest absent. That deliberate result permits only host-scoped cookies and cannot authorize cross-host post-visit attribution. Independently inspect the staged deployment metadata and confirm its deployment ID and revision belong to the reviewed build.
...
Invitation acceptance uses two same-origin JSON responses. After opening the capability delivery envelope, the email worker asks Supabase to generate a proof. Supabase returns `signup` for a new target account or `magiclink` for an existing target account, and the worker carries that exact verification type directly into the email link with the proof and independent 256 bit application capability. The no-store interstitial removes the fragment from browser history before parsing it and waits for explicit user activation. The first response passes the preserved type to `verifyOtp` and accepts only a strict successful result whose returned session and user identifiers agree. Successful implicit verification yields a provider signed session whose authentication method reference is `otp` for both email proof types. The response returns the Supabase session cookies and performs no tenant mutation. It does not add a redundant user lookup after that fresh provider response. Provider throttling, network failure, unknown provider errors, fallback session failures, and malformed provider success data remain retryable. Any queued session cookie is copied to that retryable response so a consumed proof can recover through the newly established session. Only the exact provider expired-proof outcome without a usable fresh session is deterministic. If the proof was already consumed by an ambiguous first response, retry requires a current signed session with an `otp` authentication method reference no more than 15 minutes old. The later redemption proves that the session user and confirmed email match the invitation target, then requires the independent application capability before any tenant mutation. The `otp` reference alone is never invitation authority.
...
The database migration and matching routes are a coordinated, forward-only cutover. Production provider execution for this feature is not authorized yet, so the expected legacy attempted-outbox count is zero. Any nonzero count is a release stop for investigation. The audited quarantine exists for a preview, staged, or interrupted rollout, not as permission to normalize unexplained production work.

First verify the exact hosted Supabase Email OTP Expiration setting and retain protected evidence that `mailer_otp_exp` is no more than 3,600 seconds. Supabase states that this setting governs Magic Links and other email links, including invitations. See [Passwordless email logins](https://supabase.com/docs/guides/auth/auth-email-passwordless) and [Users and invitation links](https://supabase.com/docs/guides/auth/users). Local `supabase/config.toml` is not hosted evidence. Keep the invitation worker disabled if the hosted value is unknown or greater than 3,600 seconds. Separately verify that the production Vercel plan supports the new 120 second function duration. The application uses an internal 110 second work budget, and repository configuration does not prove plan support.

Freeze approved manual callers and external schedulers, then deploy a temporary dedicated `ADVOCATE_INVITATION_EMAIL_WORKER_SECRET` that Vercel Cron does not send. Call the invitation worker with `CRON_SECRET` and prove it returns 401 before any database claim. That result blocks a new invocation but cannot prove that an invocation on the prior Production deployment has finished. From the verified 401, wait at least 70 seconds, which exceeds the legacy worker's 60 second maximum by a ten second margin, and verify in Vercel invocation telemetry that no prior worker remains active. Retain only aggregate counts for `processing` outbox rows with and without `delivery_started_at`. Do not proceed while a pre-handoff row has an unexpired five-minute claim lease. Treat every begun handoff as potentially delivered. Classify it from provider evidence, revoke it when delivery is rejected or inconclusive, or postpone the cutover. The release preflight intentionally reports `worker_configuration` as invalid while the override exists. Retain that result only as pause evidence.

Apply the migration while the worker remains disabled. It removes the legacy four-argument claim RPC and installs a version 1 shared-issuer claim that remains closed. After the migration commits, call the service-only arm RPC `public.arm_advocate_invitation_legacy_email_proof_quarantine(context_request_id uuid, context_trace_id text DEFAULT NULL) RETURNS timestamptz` as the service role. Its single contact-free result is immutable `legacy_claim_fenced_at`. Commit that transaction, wait at least 70 seconds from the returned server timestamp, then call `public.quarantine_legacy_advocate_invitation_proofs(verified_provider_otp_expiry_seconds smallint, context_request_id uuid, context_trace_id text DEFAULT NULL)`. It returns exactly `candidate_outbox_count`, `unique_recipient_count`, `quarantined_outbox_count`, `created_gate_count`, `preserved_gate_count`, `fence_expires_at`, and `executed_at`. Same-expiry replay returns the original seven contact-free fields without extending the fence. Different-expiry replay fails closed. Only the committed quarantine receipt opens version 1 claims. No older worker can satisfy the replacement signature.

The quarantine considers every possible live legacy proof across all invitation and outbox states, including sent, accepted, revoked, expired, cancelled, failed, and processing data. It also checks pre-outbox `last_sent_at`. Stop on recent contact-redacted evidence, a recent prior send without a retained recipient HMAC, incompatible normalization or HMAC versions, or any unexpected candidate or quarantined count. The verified hosted expiry from 1 through 3,600 seconds is retained as release evidence and binds replay, but it cannot shorten safety. The transaction always applies the fixed one-hour maximum proof lifetime plus the fixed 300 second safety margin, exactly 3,900 seconds, to both the legacy-evidence blocker and every recipient fence. It permanently marks unresolved attempted rows or commits nothing. Its immutable contact-free receipt returns aggregate candidate, recipient, quarantine, created-gate, and preserved-gate counts plus the authoritative `fence_expires_at` and `executed_at`. Replay returns those original facts and never extends the fence. Permanently quarantined unresolved rows require explicit revocation and reissue after the fence expires.
...
The database release gate runs `supabase/tests/advocate_invitation_email_proof_settlement.test.sql` and `yarn test:advocate:invitation-proof-quarantine-concurrency` on its fresh loopback-only Supabase stack before the existing initial-owner and publication concurrency gates. Its separately uploaded sanitized artifact proves the committed arm-before-wait sequence, the database-enforced 70 second drain, exact immutable quarantine replay, changed-expiry rejection, version 1 claim closure until quarantine commits, exact recipient-fence boundaries, and quarantine-first serialization against invitation creation, claim, settlement, revocation, and retention work. It also proves that an ordinary reader or a prior settlement, revocation, or lifecycle writer causes exact fail-fast SQLSTATE `55P03`, leaves no partial cutover mutation, and permits a clean quarantine retry after the competing transaction ends. It may contain scenario names, fixed outcome categories, aggregate counts, migration provenance, and pass state. It must not contain contact, recipient HMACs, credentials, invitation or outbox identifiers, operation UUIDs, trace values, or raw audit material.

Deploy the shared-issuer application only after the receipt commits. The outbox UUID is the shared-gate operation ID. The worker stores no proof material, uses exact shared-issuer delays with no exponential multiplier, refunds provider-free outcomes, retains provider ambiguous and issued-not-handed-off attempts, and keeps the existing five-minute claim lease. Batch size equals concurrency so claimed work cannot wait in a local queue. With the dedicated secret, validate one new-account invitation and one existing-account invitation end to end. Remove the override in a new deployment, verify Vercel Cron resumes, and only then retain a configured release preflight. On rollback, keep the migration and quarantine receipt, never restore the retired claim RPC or direct proof worker, and keep delivery disabled until a compatible shared-issuer build returns.

Supabase Auth v2.188.1 uses the `otp` authentication method reference for implicit verification sessions across email and phone proof types. The MVP threat model therefore requires the hosted phone authentication provider itself to be disabled, with phone MFA enrollment and phone MFA verification also disabled. Verify all three hosted controls directly before release. The local configuration disables the Twilio phone provider and both phone MFA operations, but cannot prove hosted state. Email recovery, invite, and email-change OTP continue to prove email control. Invitation redemption independently requires the current confirmed email to equal the invitation target, the exact target user, and the application capability. Enabling any phone authentication path requires a purpose-specific authentication receipt that distinguishes its delivery channel.

FF-029 remains `in_progress`. The schema v3 loopback harness under `tests/provider` defines 86 sanitized scenarios, synchronizes both requests at the Auth dispatch boundary, repeats every concurrent pair three times in both consumption orders, and fails closed on classified provider failures, identity mismatch, deadline exhaustion, incomplete provenance, or cleanup failure. Cleanup removes only tracked canary users and messages and preserves unrelated Mailpit mail. A hardened local run against Supabase Auth v2.188.1 and CLI 2.90.0 completed in 59,960 milliseconds with cleanup complete. It fully observed 67 scenarios, categorized 18 as `unexpected_failure`, and retained expiry as the one explicit local placeholder. Every failure occurred during simultaneous new-account issuance, where exactly one proof was issued and accepted while the other issuance failed. This proves only that concurrent local account creation is unsafe. It does not establish a reliable winner or hosted Supabase behavior. The earlier 36-scenario and interim 54-scenario results are not retained because their classification, cleanup, or actual dispatch synchronization was insufficient. Run and retain the complete hosted production-equivalent matrix, including expiry, before release.

FF-042 remains `in_progress` until all required evidence is retained. Automated evidence must cover exact request parsing, same-origin and no-store controls, source and global authentication quotas, quiet-traffic retention, session establishment before mutation, refresh cookie propagation, initial-owner and delegate receipts, exact replay, changed-input rejection, wrong-user nondisclosure, revoked-session and banned-account rejection, lost and malformed responses, reload recovery, operation precedence, reauthentication, and the initial-owner redeem-versus-recover concurrency interleaving. Retained local evidence includes 142 focused invitation and owner assertions, 57 pgTAP files with 1,957 assertions, 11 concurrency scenarios with 16 deterministic interleavings, 135 relevant Chromium tests, 19 mobile WebKit scenarios, type checking, lint, and the production build. The remaining device evidence is a current physical iOS smoke test that opens the link from Mail, backgrounds and restores the tab, reloads or returns after process eviction, completes sign-in recovery in another tab, and proves that no invitation secret or session value entered localStorage, sessionStorage, request targets, referrers, analytics, or logs.
