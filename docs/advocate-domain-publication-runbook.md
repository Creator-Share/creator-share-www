...
The database migration and matching routes are a coordinated cutover. Freeze approved manual callers and external schedulers, then deploy a temporary dedicated `ADVOCATE_INVITATION_EMAIL_WORKER_SECRET` that Vercel Cron does not send. Call the invitation worker with `CRON_SECRET` and prove it returns 401 before any database claim. That result blocks a new invocation but cannot prove that an invocation on the prior Production deployment has finished. From the verified 401, wait at least 70 seconds, which exceeds the route's 60 second maximum, and verify in Vercel invocation telemetry that no prior invitation worker remains active. Through protected operational database access, retain only aggregate counts for `processing` outbox rows with and without `delivery_started_at`. Do not proceed while a pre-handoff row still has an unexpired five-minute claim lease. Treat every begun handoff as potentially delivered and follow the invitation ambiguity procedure. Classify it from provider evidence, revoke it before proceeding when delivery is rejected or remains inconclusive, or postpone the cutover. Only then is automated delivery quiescent. No invitation acceptance gate exists, so already issued links remain live under the deployed application behavior. The release preflight intentionally reports `worker_configuration` as invalid while this override exists. Retain that result only as pause evidence, never as the normal release preflight. Apply the additive forward-only database correction that recognizes the provider issued `otp` authentication method, then deploy the compatible application that carries exact `signup` or `magiclink` generation types through `verifyOtp`. Using an approved request authenticated with the dedicated worker secret, validate one new-account invitation and one existing-account invitation end to end. Remove the override in a new deployment, verify Vercel Cron delivery resumes, and only then retain a configured release preflight. On application rollback, keep the database correction and restore and drain the delivery pause before changing builds. An old application fails already issued `signup` links safely. Repair forward or safely revoke and reissue those links after the compatible build returns. Do not roll back the `otp` correction.
