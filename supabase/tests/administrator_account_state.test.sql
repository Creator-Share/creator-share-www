...
SELECT extensions.is((SELECT count(*) FROM public.list_my_recurring_sponsorships()),1::bigint,'expired sponsor ban restores owned subscription access');
RESET ROLE;
...
SELECT extensions.throws_ok('SELECT * FROM public.list_my_recurring_sponsorships()','42501','Recurring sponsorships require an authenticated account','soft-deleted sponsor cannot read recurring sponsorships');
RESET ROLE;
